...
def get_connection_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "key_filename": os.getenv(
            "SLURM_KEY_FILE", os.path.expanduser("~/.ssh/id_rsa")
        ),
...
def squeue(
    ...
    try:
        ...
        result = client.squeue(
            user=user, job_id=job_id, partition=partition, format_str=format_str
        )
...
def disconnect() -> Dict[str, str]:
    ...
    try:
        ...
        return {"status": "success", "message": "Disconnected from SLURM cluster"}
    except Exception as e:
        return {"status": "error", "error": str(e)}
